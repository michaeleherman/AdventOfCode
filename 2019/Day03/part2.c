...
int main()
{
    ...
    struct wireArrayStruct firstWire = getWireArray(wires0);
    struct wireArrayStruct secondWire = getWireArray(wires1);

    int **wire0Array = firstWire.tmpArray;
    int wire0Counter = firstWire.counter;
    int **wire1Array = secondWire.tmpArray;
    int wire1Counter = secondWire.counter;

    // printf("wire 0 output.\n\n");
    // for (int i = 0; i < wire0Counter; i++)
    // {
    //     printf("%d, %d: %d\n", wire0Array[i][0], wire0Array[i][1], wire0Array[i][2]);
    // }
    // getchar();

    // printf("wire 1 output.\n\n");
    // for (int i = 0; i < wire1Counter; i++)
    // {
    //     printf("%d, %d: %d\n", wire1Array[i][0], wire1Array[i][1], wire1Array[i][2]);
    // }
    // // getchar();
    ...
    for (int i = 0; i < wire0Counter; i++)
    {
        int wire0X = wire0Array[i][0];
        int wire0Y = wire0Array[i][1];
        int wire0Length = wire0Array[i][2];

        // printf("wire0: %d,%d %d - ", wire0X, wire0Y, wire0Length);

        for (int j = 0; j < wire1Counter; j++)
        {
            int wire1X = wire1Array[j][0];
            ...
            if (wire0X == wire1X)
            {
                ...
                {
                    // printf("wire1: %d,%d %d\n", wire1X, wire1Y, wire1Length);
                    if (wire1X == 0 && wire1Y == 0)
                    {
                        continue;
                    }
                    int tmpTaxiDistance = abs(wire1X) + abs(wire1Y);
                    int tmpLineLength = wire0Length + wire1Length;
                    printf("%d Found a match!! %d, %d: %d %d\n", j, wire1X, wire1Y, tmpTaxiDistance, tmpLineLength);
                    (tmpLineLength < shortestDistance) && (shortestDistance = tmpLineLength);
                    // j = pointsCounter;
                }
            }
        }
