...
int main() {
    ...
    while (fgets(chunk, sizeof(chunk), fp) != NULL) {
        ...
        if (strstr(chunk,"mask")) {
            ...
            record.xpositions = malloc(sizeof(int));
            ...
            if (record.location > maxMemLocation) {
                *memArr = realloc(memArr, sizeof(long long) * (record.location + 2));
                ...
                for (long long i = oldMax+1; i<maxMemLocation; i++) {
                    *memArr[i] = 0;
                }
            }
...
void makeMaskArray (char chunk[chunkLength], int maskArray[BITS]) {
    ...
    for(int i = 0; i < BITS;i++){
        if (tmpChunk[i] == 49 ) {
            ...
            record.xpositions[xPositionsEnd] = i;
            record.xpositions = realloc(record.xpositions,sizeof(int) * (xPositionsEnd + 2));
